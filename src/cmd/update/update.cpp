...
std::string fetchLatestTag(bool stable_only)
{
    ...
    for (auto& r : releases)
    {
        ...
        if (stable_only && r.value("prerelease", true))
            continue;

        if (latest.is_null() ||
            r.value("published_at", "") >
            latest.value("published_at", ""))
        {
            latest = r;
        }
    }
...
UpdateMode choiceToMode(int choice)
{
    switch (choice)
    {
        ...
        default:
            throw std::runtime_error("Invalid update choice");
    }
}
...
void perform(UpdateMode mode, const std::string& gitTag)
{
    ...
    if (mode == UpdateMode::Git)
    {
        ...
        std::string cloneCmd =
            ...
            REPO + ".git " + repo_dir.string();

        console::spinner("Cloning repository...", cloneCmd);

        if (!fs::exists(src_dir))
        {
            std::cerr << "[!] nucleus-shell directory missing in repo\n";
            return;
        }
        ...
        std::system("killall quickshell &>/dev/null || true");
        std::system("nohup quickshell -c nucleus-shell &>/dev/null & disown");

        std::cout << "[✓] Installed nucleus-shell from Git\n";
    ...
    {
        if (gitTag.empty())
            throw std::runtime_error("Tag mode requires a tag");
    ...
    if (!fs::exists(src_dir))
    {
        std::cerr << "[!] nucleus-shell directory missing in archive\n";
        return;
    }
